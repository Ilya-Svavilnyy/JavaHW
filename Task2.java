        ...
        for (int current = 0; current < size_arr - 1; current++) {
            for (int i = 0; i < (size_arr - 1 - current); i++) {
                if (num_arr[i] > num_arr[i + 1]){
                    ...
                    msg = String.format("%d. Array changed %s \n", count++, Arrays.toString(num_arr));
                } else { msg = String.format("%d. Array dont change after last iteration \n", count++); }
                 
                journal.log(Level.INFO, msg);
            }
        }
        msg = String.format("Created %d operations.\nSort result  - %s", --count, Arrays.toString(num_arr));
        journal.log(Level.INFO, msg);
    }
}
